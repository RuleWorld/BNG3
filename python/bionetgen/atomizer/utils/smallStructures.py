...
def parseReactions(reaction):
    ...
    molecule = Word(alphanums + "_") + Optional(
        Suppress("(")
        + Group(components)
        + ZeroOrMore(Suppress(",") + Group(components))
        + Suppress(")")
    )
...
class Rule:
    ...
    def __str__(self):
        ...
        finalStr += (
            " + ".join([str(x) for x in self.reactants])
            + arrow
...
class Species:
                ...
                for index in range(0, len(component.bonds)):
                    if int(component.bonds[index]) in intersection:
                        if component.bonds[index] in correspondence:
                            ...
                            correspondence[component.bonds[index]] = (
                                max(intersection) + 1
                            )
    ...
    def extractAtomicPatterns(self, action, site1, site2, differentiateDimers=False):
        ...
        for molecule in self.molecules:
            moleculeCounter = (
                nameCounter[molecule.name] - nameCounterCopy[molecule.name]
            )
                ...
                if component.activeState != "":
                    ...
                    if differentiateDimers:
                        moleculeStructure = Molecule(
                            molecule.name + "%{0}".format(moleculeCounter), molecule.idx
                        )
                    else:
    ...
    def graphVizGraph(self, graph, identifier, layout="LR", options={}):
        ...
        for idx, molecule in enumerate(self.molecules):
            ...
            if len(self.molecules) == 1:
                compDictionary = molecule.graphVizGraph(
                    graph, ident, flag=False, options=options
                )
            else:
        ...
        for bond in self.bonds:
            if bond[0] in speciesDictionary and bond[1] in speciesDictionary:
                if layout == "RL":
                    graph.add_edge(
                        speciesDictionary[bond[1]],
                        speciesDictionary[bond[0]],
                        dir="none",
                        len=0.1,
                        weight=100,
                    )
    ...
    def listOfBonds(self, nameDict):
        ...
        for bond in self.bonds:
            ...
            listofbonds[nameDict[mol1]][nameDict[bond[0]]] = [
                (nameDict[mol2], nameDict[bond[1]])
            ]
            if nameDict[mol2] not in listofbonds:
                listofbonds[nameDict[mol2]] = {}
            listofbonds[nameDict[mol2]][nameDict[bond[1]]] = [
                (nameDict[mol1], nameDict[bond[0]])
            ]
...
class Molecule:
    ...
    def graphVizGraph(self, graph, identifier, components=None, flag=False, options={}):
        ...
        if len(self.components) == 0:
            ...
            s1.add_node(
                "cluster%s_%s_dummy" % (identifier, self.idx),
                shape="point",
                style="invis",
            )
            if components == None:
...
class Component:
    ...
    def graphVizGraph(self, graph, identifier):
        ...
        if len(self.states) == 0:
            ...
            s1 = graph.subgraph(
                name="cluster%s_%s" % (identifier, self.idx), label=self.name
            )
            if self.activeState != "":
                ident = "%s" % (identifier)
