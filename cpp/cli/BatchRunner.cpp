...
fs::path makeOutputRoot(const fs::path& requested) {
    if (!requested.empty()) {
        ...
        if (error) {
            throw std::runtime_error("could not create parallel output root " + root.string()
                                     + ": " + error.message());
        }
        ...
        if (error) {
            throw std::runtime_error("could not inspect parallel output root " + root.string()
                                     + ": " + error.message());
        }
        return root;
    }
...
fs::path resolveExecutable(const fs::path& executable) {
    if (executable.empty()) throw std::runtime_error("parallel executable path is empty");
...
std::vector<Job> stageJobs(const ParallelBatchOptions& options, const fs::path& outputRoot) {
    ...
    for (std::size_t index = 0; index < options.inputs.size(); ++index) {
        ...
        jobs.push_back({workingDirectory, localModelPath});
    }
    return jobs;
}

std::vector<std::string> childArguments(const fs::path& executable,
                                        const fs::path& localModelPath,
                                        bool checkOnly, bool verbose) {
    std::vector<std::string> arguments;
    arguments.reserve(4);
    arguments.push_back(executable.string());
    if (checkOnly) arguments.emplace_back("--check");
    if (verbose) arguments.emplace_back("--verbose");
    arguments.push_back(localModelPath.string());
    return arguments;
}

int runChild(const fs::path& executable, const Job& job, bool checkOnly, bool verbose,
    ...
    for (auto& argument : arguments) nativeArguments.push_back(argument.data());
    ...
    if (process == 0) {
        if (chdir(job.workingDirectory.c_str()) != 0) _exit(126);
        if (executable.has_parent_path()) {
                ...
                while (true) {
                    ...
                    try {
                        results[index].exitCode = runChild(
                            executable, jobs[index], options.checkOnly, options.verbose,
                            results[index].error);
